...
func commit_handle(gizmo: EditorNode3DGizmo, index: int, restore, cancel := false):
	var node : SDFCylinder = gizmo.get_spatial_node()
	var ur := _undo_redo
...
func redraw(gizmo: EditorNode3DGizmo):
	...
	for i in 4:
		var p := radius * Vector2.from_angle(lines_angle_start + float(i) * lines_angle_step)
	...
	gizmo.add_handles(PackedVector3Array(handles), get_material("handles_billboard", gizmo), [], false)
